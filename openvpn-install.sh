...
# SJ modifies
  cp /etc/iptables/* /root/
  echo "#!/bin/bash -x
	if [ ! -e /dev/net/tun ]; then
    mkdir -p /dev/net && mknod /dev/net/tun c 10 200
	fi

  /root/rm-openvpn-rules.sh
  /root/add-openvpn-rules.sh
  /usr/sbin/openvpn --cd /etc/openvpn --config /etc/openvpn/server.conf" > /root/openvpn-run.sh
  chmod +x /root/*.sh
